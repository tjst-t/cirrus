...
- Total: 40 Sprints | Done: 14 | In Progress: 0 | Remaining: 26
- [████████░░░░░░░░░░░░] 35%

## Execution Order

S001 → S002 → S003 → S004 → S005 → S006 → S007 → S008 → S009 → S010 → S011 → S012 → S013 → S014 → S015 → S016 → S017 → S018 → S019 → S020 → S021 → S022 → S023 → S024 → S025 → S026 → S027 → S028 → S029 → S030 → S031 → S032 → S033 → S034 → S035 → S036 → S037 → S038 → S039 → S040
                                                                                                                                         ↑ next
...
## Sprint S015: Scheduler + VM 作成 [ ]

VM 作成 API が動作し、スケジューラがホストとバックエンドを選定し、cirrus-sim 上で VM が起動する。

### Story S015-0: S014 動作確認（make serve + make serve-storage） [ ]

S014 で実装した iSCSI / RBD ドライバーを実際のコンテナ環境で動作確認する。ユニットテスト・ビルドは済んでいるが、実行時の結合テストは未実施。

- [ ] **Task S015-0-1**: `make serve` でコントローラー + ワーカーが正常起動すること（ログ確認・`/healthz` 疎通確認）
- [ ] **Task S015-0-2**: `make serve-storage` で iSCSI target コンテナ（10.100.0.100:8080）が起動し `/healthz` が 200 を返すこと
- [ ] **Task S015-0-3**: `make serve-storage` で Ceph コンテナ（10.100.0.101:8090）が起動し `/healthz` が 200 を返すこと
- [ ] **Task S015-0-4**: `go test -tags integration -run TestISCSIDriver ./test/integration/...` が PASS すること
- [ ] **Task S015-0-5**: `go test -tags integration -run TestRBDDriver ./test/integration/...` が PASS すること

### Story S015-1: Flavor エンティティ [ ]

- [ ] **Task S015-1-1**: マイグレーション: flavors, flavor_access テーブル
- [ ] **Task S015-1-2**: `internal/flavor/models.go, service.go, store.go` Flavor CRUD
- [ ] **Task S015-1-3**: 管理者 API: POST/GET/PUT/DELETE /api/v1/flavors
- [ ] **Task S015-1-4**: テナント API: GET /api/v1/flavors（利用可能な Flavor 一覧）
- [ ] **Task S015-1-5**: CLI: `cirrusctl admin flavor create/list/show` + `cirrusctl flavor list`
- [ ] **Task S015-1-6**: `make serve` でデフォルト Flavor シード（m1.small/medium/large）

### Story S015-2: Scheduler [ ]

- [ ] **Task S015-2-1**: `internal/scheduler/scheduler.go` Scheduler インターフェース
- [ ] **Task S015-2-2**: フィルタリング: AZ フィルタ、Flavor→Capability マッチング、稼働状態フィルタ
- [ ] **Task S015-2-3**: スコアリング: ホストのリソース空き率、バックエンドの容量空き率
- [ ] **Task S015-2-4**: Schedule() → (host_id, backend_id) ペア返却

### Story S015-3: BlockDev（Worker 側） [ ]

- [ ] **Task S015-3-1**: `internal/blockdev/manager.go` Manager インターフェース
- [ ] **Task S015-3-2**: Attach/Detach 実装（ExportInfo の protocol に応じた処理）
- [ ] **Task S015-3-3**: cirrus-sim では protocol="rbd" のスタブ接続

### Story S015-4: Hypervisor VM 操作 [ ]

- [ ] **Task S015-4-1**: `internal/hypervisor/libvirt/libvirt.go` DefineVM, StartVM, StopVM, DestroyVM, UndefineVM
- [ ] **Task S015-4-2**: domain XML 生成（テンプレートベース）: ディスク、ポート（interfaceid）、cloud-init
- [ ] **Task S015-4-3**: cloud-init ISO 生成（network-config, meta-data, user-data）

### Story S015-5: gRPC CreateVM [ ]

- [ ] **Task S015-5-1**: proto/agent.proto CreateVM RPC 追加（DiskSpec、PortSpec）
- [ ] **Task S015-5-2**: worker 側: CreateVM → BlockDev.Attach → Hypervisor.DefineVM → StartVM

### Story S015-6: Compute Orchestrator [ ]

- [ ] **Task S015-6-1**: `internal/compute/service.go` Service インターフェース
- [ ] **Task S015-6-2**: `internal/compute/orchestrator.go` CreateVM（Network.CreatePort → Storage.CreateVolume → Scheduler.Schedule → Storage.ExportVolume → Agent.CreateVM → Network.BindPort）
- [ ] **Task S015-6-3**: 非同期ジョブ実行（goroutine + channel）
- [ ] **Task S015-6-4**: マイグレーション: vms, vm_volumes テーブル

### Story S015-7: API + テスト + CLI [ ]

- [ ] **Task S015-7-1**: POST /api/v1/vms（202 Accepted）: flavor_id, az, network_id, volume_type_id 指定
- [ ] **Task S015-7-2**: GET /api/v1/vms, GET /api/v1/vms/{id}
- [ ] **Task S015-7-3**: POST /api/v1/volumes/{id}/attach, /detach（S013-4-4 からの移行）
- [ ] **Task S015-7-4**: 結合テスト: VM 作成→スケジューラ→worker→cirrus-sim でドメイン起動
- [ ] **Task S015-7-5**: `cirrusctl vm create/list/show`

---

## Sprint S016: VM ライフサイクル [ ]

VM の起動・停止・再起動・削除が動作し、ステータス遷移が正しく管理される。

### Story S016-1: VM 操作 gRPC [ ]

- [ ] **Task S016-1-1**: proto/agent.proto DeleteVM, StartVM, StopVM, RebootVM, GetVMState RPC 追加
- [ ] **Task S016-1-2**: worker 側: 各操作の Hypervisor 委譲実装

### Story S016-2: Compute Orchestrator 操作 [ ]

- [ ] **Task S016-2-1**: DeleteVM: DestroyVM → UndefineVM → BlockDev.Detach → Storage.UnexportVolume → DeleteVolume → Network.DeletePort → DB 更新
- [ ] **Task S016-2-2**: StartVM / StopVM / RebootVM: gRPC 経由で worker に指示
- [ ] **Task S016-2-3**: 削除時のリソース解放順序の保証

### Story S016-3: ステータス管理 [ ]

- [ ] **Task S016-3-1**: VM ステータス遷移の状態機械実装
- [ ] **Task S016-3-2**: エラー時のステータス遷移（building→error 等）
- [ ] **Task S016-3-3**: 非同期ジョブ失敗時のクリーンアップ（作成途中リソース削除）

### Story S016-4: テスト + CLI [ ]

- [ ] **Task S016-4-1**: 結合テスト: 作成→停止→起動→再起動→削除の全ライフサイクル
- [ ] **Task S016-4-2**: 異常系: 作成途中の worker 障害で error ステータス遷移
- [ ] **Task S016-4-3**: cirrus-sim 障害注入: libvirt-sim の DomainCreate 失敗
- [ ] **Task S016-4-4**: `cirrusctl vm start/stop/reboot/delete`

---

## Sprint S017: ホスト状態遷移制約 + Heartbeat 監視 [ ]

ホストの operational_state 遷移に制約を適用し、heartbeat 途絶で faulty 自動遷移、draining 完了で maintenance 自動遷移が動作する。設計: docs/host.md 参照。

### Story S017-1: 状態遷移制約 [ ]

- [ ] **Task S017-1-1**: SetOperationalState に遷移ルール適用（host.md の遷移表に準拠）
- [ ] **Task S017-1-2**: retiring は終端状態（activate 不可）
- [ ] **Task S017-1-3**: active→maintenance は稼働 VM 数=0 の場合のみ許可
- [ ] **Task S017-1-4**: 不正な遷移は 409 Conflict で拒否

### Story S017-2: Heartbeat 監視 + faulty 自動遷移 [ ]

- [ ] **Task S017-2-1**: `internal/controller/heartbeat_monitor.go` 定期的に last_heartbeat を監視
- [ ] **Task S017-2-2**: 3回連続タイムアウト（デフォルト 30 秒無応答）で active/draining → faulty

### Story S017-3: HostFaultyHandler（カスケード障害処理） [ ]

- [ ] **Task S017-3-1**: `internal/controller/host_faulty_handler.go` faulty 遷移時のカスケード更新
- [ ] **Task S017-3-2**: faulty 遷移直後: ホスト上の全 VM を error に、関連ポートを down に更新

### Story S017-4: Draining 完了の自動遷移 + テスト [ ]

- [ ] **Task S017-4-1**: draining 状態のホストで稼働 VM 数が 0 になったら maintenance に自動遷移
- [ ] **Task S017-4-2**: テスト: 不正遷移拒否、heartbeat 途絶→faulty 自動遷移
- [ ] **Task S017-4-3**: テスト: faulty→HostFaultyHandler→VM/ポートカスケード更新
- [ ] **Task S017-4-4**: テスト: draining→VM 退避完了→maintenance 自動遷移

---

## Sprint S018: DriftEvent 基盤 + Heartbeat Reconciler [ ]
...
### Story S018-1: DriftEvent 基盤 [ ]

- [ ] **Task S018-1-1**: `internal/controller/reconcile/drift.go` DriftEvent 型定義
- [ ] **Task S018-1-2**: DriftHandler: Deduplicator（インメモリ TTL キャッシュ）+ Logger/AlertSink + AutoHealer
- [ ] **Task S018-1-3**: マイグレーション: drift_events テーブル
- [ ] **Task S018-1-4**: reconcile 設定パラメータ（--reconcile-interval, --auto-heal-enabled 等）

### Story S018-2: Heartbeat Reconciler [ ]

- [ ] **Task S018-2-1**: ResourceReport に RunningVMs フィールド追加
- [ ] **Task S018-2-2**: `internal/controller/reconcile/compute.go` HeartbeatReconciler
- [ ] **Task S018-2-3**: DB有・heartbeat無 → Auto-heal: DB→error（楽観的ロック）
- [ ] **Task S018-2-4**: DB無・heartbeat有 → Alert
- [ ] **Task S018-2-5**: VM ステータス不一致: ユーザ操作記録の有無で分岐

### Story S018-3: 既存 Reconciler 移行 + テスト [ ]

- [ ] **Task S018-3-1**: Network ReconcilerをDriftEvent 発火に移行
- [ ] **Task S018-3-2**: Storage ReconcilerをDriftEvent 発火に移行
- [ ] **Task S018-3-3**: テスト: DriftEvent 永続化、HeartbeatReconciler 各ケース、重複抑制
...
### Story S019-1: Quota Service [ ]

- [ ] **Task S019-1-1**: `internal/quota/service.go` Service インターフェース
- [ ] **Task S019-1-2**: Check, Reserve, Commit, Release 実装（予約パターン）
- [ ] **Task S019-1-3**: 組織クォータとテナントクォータの両方を検査

### Story S019-2: クォータ対象リソース + 統合 [ ]

- [ ] **Task S019-2-1**: vCPU, メモリ, VM 数, ボリューム数/容量, スナップショット数、ネットワーク数, egress/ingress 数
- [ ] **Task S019-2-2**: Compute.CreateVM に Quota.Check + Reserve/Commit/Release 組み込み（S015 スタブを本実装に差し替え）
- [ ] **Task S019-2-3**: Storage.CreateVolume / Network.CreateNetwork にクォータチェック組み込み
- [ ] **Task S019-2-4**: PUT/GET /api/v1/tenants/{id}/quota
- [ ] **Task S019-2-5**: 結合テスト: クォータ上限設定→超過で 403 拒否
- [ ] **Task S019-2-6**: `cirrusctl quota show/set`

---

## Sprint S020: Egress + Ingress + ゲートウェイ [ ]

テナント Network からの外部接続（Egress）と外部からの着信（Ingress）がゲートウェイノード経由で動作する。設計: docs/network.md 参照。

### Story S020-1: ゲートウェイノード管理 [ ]

- [ ] **Task S020-1-1**: 管理者 API: POST/GET/DELETE /gateway-nodes
- [ ] **Task S020-1-2**: GW ノードのステータス管理（active, draining, retired）
- [ ] **Task S020-1-3**: Network 単位で GW ノードペア（Active-Standby）を割り当て

### Story S020-2: Egress [ ]

- [ ] **Task S020-2-1**: テナント API: POST/GET/DELETE /tenants/{tid}/networks/{nid}/egresses
- [ ] **Task S020-2-2**: Egress タイプ: NAT Gateway（SNAT）、VPN（IPsec/WireGuard）、Direct Connect（VLAN trunk）
- [ ] **Task S020-2-3**: HostNetworkState に Egress ルールを含めて配信
- [ ] **Task S020-2-4**: エージェント側: Egress 宛フローを GW ノードへ転送

### Story S020-3: Ingress [ ]

- [ ] **Task S020-3-1**: テナント API: POST/GET/DELETE /tenants/{tid}/networks/{nid}/ingresses
- [ ] **Task S020-3-2**: Ingress タイプ: Direct IP（パブリック IP を特定 VM に 1対1 DNAT）、L4 Load Balancer（conntrack + DNAT で分散）
- [ ] **Task S020-3-3**: 管理者 API: /ip-pools（パブリック IP プール管理）
- [ ] **Task S020-3-4**: L4 LB: conntrack セッションアフィニティ、コントローラ主導ヘルスチェック

### Story S020-4: 内部 LB + テスト [ ]

- [ ] **Task S020-4-1**: テナント API: POST/GET/DELETE /tenants/{tid}/networks/{nid}/load-balancers
- [ ] **Task S020-4-2**: Group に VIP 割り当て、各ホストの OVS で分散実行
- [ ] **Task S020-4-3**: Reconciler 拡張: Egress/Ingress のデータプレーン照合 → DriftEvent 発火
- [ ] **Task S020-4-4**: テスト: NAT GW Egress、Direct IP + L4 LB Ingress、内部 LB
- [ ] **Task S020-4-5**: `cirrusctl egress/ingress/load-balancer/admin gateway-node`

---

## Sprint S021: Phase 1 安定化 [ ]

Phase 1 全機能の結合テストが通り、安定してデプロイできる状態。

### Story S021-1: E2E テストスイート [ ]

- [ ] **Task S021-1-1**: テナント作成→VM 削除のフルフロー E2E テスト
- [ ] **Task S021-1-2**: マルチテナントシナリオ（テナント A/B の隔離確認）
- [ ] **Task S021-1-3**: Policy/Group によるアクセス制御確認
- [ ] **Task S021-1-4**: cirrus-sim medium 環境でのテスト

### Story S021-2: エラーハンドリング + API 仕上げ [ ]

- [ ] **Task S021-2-1**: 全 API エンドポイントのバリデーション強化
- [ ] **Task S021-2-2**: 非同期ジョブの失敗時クリーンアップ網羅
- [ ] **Task S021-2-3**: ページネーション・フィルタリング・ソート（全リスト系 API）

### Story S021-3: Reconcile 結合テスト [ ]

- [ ] **Task S021-3-1**: cirrus-sim で VM 状態不整合を注入 → HeartbeatReconciler が DriftEvent 発火 → Auto-heal 動作確認
- [ ] **Task S021-3-2**: OVS フロー手動削除 → Network Reconciler 検出 → Alert 確認
- [ ] **Task S021-3-3**: heartbeat 停止 → faulty → HostFaultyHandler → VM/ポートカスケード更新

---

## Sprint S022: ライブマイグレーション [ ]

同一コンピュートプール内で VM をライブマイグレーションできる。Fallback パターンによるゼロパケットロス移行。

### Story S022-1: Hypervisor + gRPC Migration [ ]

- [ ] **Task S022-1-1**: `internal/hypervisor/libvirt/libvirt.go` MigrateVM 実装（DomainMigratePerform3Params 等）
- [ ] **Task S022-1-2**: proto/agent.proto PrepareMigration, StartMigration RPC
- [ ] **Task S022-1-3**: worker 側: migration 準備→実行→完了の3フェーズ

### Story S022-2: Scheduler Reschedule + Compute Orchestrator [ ]

- [ ] **Task S022-2-1**: `internal/scheduler/scheduler.go` Reschedule 実装（移行元ホストの除外）
- [ ] **Task S022-2-2**: MigrateVM: Reschedule → PrepareMigration → StartMigration → Network.BindPort 更新

### Story S022-3: Fallback パターン [ ]

- [ ] **Task S022-3-1**: 移動先ホストにフロー + ポート準備
- [ ] **Task S022-3-2**: 移動元ホストに Fallback 転送設定（移動先への Geneve 転送）
- [ ] **Task S022-3-3**: 他ホストのトンネル宛先更新 + ACK 管理（タイムアウト 30 秒）
- [ ] **Task S022-3-4**: ポート状態遷移: active→migrating→switching→draining→active

### Story S022-4: テスト + CLI [ ]

- [ ] **Task S022-4-1**: POST /api/v1/vms/{id}/actions (action=migrate)
- [ ] **Task S022-4-2**: 結合テスト: VM 作成→マイグレーション→移行先で稼働確認
- [ ] **Task S022-4-3**: cirrus-sim 障害注入: マイグレーション中の失敗→Fallback で元ホスト継続
- [ ] **Task S022-4-4**: `cirrusctl vm migrate`

---

## Sprint S023: HA Failover [ ]

ホスト障害検出時にフェンシングを行い、影響 VM を別ホストで自動再起動する。設計: docs/reconciliation.md 参照。

### Story S023-1: FencingAgent [ ]

- [ ] **Task S023-1-1**: `internal/controller/fencing/agent.go` FencingAgent インターフェース
- [ ] **Task S023-1-2**: hook 経由（AWX）で IPMI power-off 実行 + 電源 OFF 確認のポーリング
- [ ] **Task S023-1-3**: フェンシングタイムアウト + 失敗時 Alert(critical)

### Story S023-2: FailoverTrigger + VM 再起動フロー [ ]

- [ ] **Task S023-2-1**: `internal/controller/reconcile/failover.go` FailoverTrigger 実装
- [ ] **Task S023-2-2**: faulty 遷移 → FencingAgent → 成功 → Reschedule → VM 再起動
- [ ] **Task S023-2-3**: error 状態 VM の Reschedule → Storage.ReexportVolume → Agent.CreateVM → Network.RebindPort

### Story S023-3: テスト [ ]

- [ ] **Task S023-3-1**: worker 停止 → faulty → フェンシング → failover → 別ホストで VM 再起動
- [ ] **Task S023-3-2**: フェンシング失敗シナリオ: failover 中止 + Alert 発火
- [ ] **Task S023-3-3**: 複数 VM 同時 failover: 全 VM が順次別ホストに再配置

---

## Sprint S024: DRS [ ]

コンピュートプール内のリソース偏りを検出し、自動で再配分する。

### Story S024-1: DRS エンジン + 実行 [ ]

- [ ] **Task S024-1-1**: DRS ポリシー定義（CPU/メモリ使用率の標準偏差閾値、実行間隔、最大同時マイグレーション数）
- [ ] **Task S024-1-2**: `internal/scheduler/drs.go` リソース偏りの検出 + マイグレーション計画生成
- [ ] **Task S024-1-3**: 定期実行（configurable interval）、同時マイグレーション数制限
- [ ] **Task S024-1-4**: テスト: ホスト間のリソース偏り→DRS 実行→分布改善確認

---

## Sprint S025: ホストプロファイル + Hook [ ]

ホストプロファイルを定義し、AWX hook でホストに適用できる。ロールアウトが動作する。

### Story S025-1: プロファイルモデル + Hook Executor [ ]

- [ ] **Task S025-1-1**: マイグレーション: host_profiles テーブル、hosts.profile_id, hosts.profile_status
- [ ] **Task S025-1-2**: `internal/hook/awx/awx.go` AWX REST API 実装（ジョブ実行 + ポーリング + パラメータマッピング）
- [ ] **Task S025-1-3**: cirrus-sim awx-sim への接続テスト

### Story S025-2: Profile Service + Rollout [ ]

- [ ] **Task S025-2-1**: `internal/host/profile.go` CreateProfile, ApplyProfile（Hook.Execute → profile_status 更新）
- [ ] **Task S025-2-2**: `internal/host/rollout.go` StartRollout（フォルトドメイン単位のカナリアデプロイ）
- [ ] **Task S025-2-3**: batch_size, pause_between_batches、ロールバック条件（ヘルスチェック失敗率）
- [ ] **Task S025-2-4**: POST/GET /api/v1/host-profiles, POST /rollout
- [ ] **Task S025-2-5**: 結合テスト: プロファイル作成→ロールアウト→awx-sim でジョブ実行確認
- [ ] **Task S025-2-6**: `cirrusctl host-profile/rollout`

---

## Sprint S026: スナップショット + クローン [ ]

ボリュームのスナップショット取得、スナップショットからのクローン作成、依存関係管理が動作する。

### Story S026-1: スナップショットモデル + Service 拡張 [ ]

- [ ] **Task S026-1-1**: マイグレーション: snapshots テーブル、volumes.parent_snapshot_id
- [ ] **Task S026-1-2**: CreateSnapshot → BackendDriver.CreateSnapshot + DB
- [ ] **Task S026-1-3**: DeleteSnapshot → 依存関係チェック（子クローンあれば拒否）→ BackendDriver.DeleteSnapshot
- [ ] **Task S026-1-4**: CloneFromSnapshot → BackendDriver.CloneSnapshot + DB

### Story S026-2: 依存関係グラフ + API + CLI [ ]

- [ ] **Task S026-2-1**: ボリューム→スナップショット→クローンの親子関係管理
- [ ] **Task S026-2-2**: フラット化操作（非同期）: 子をフルコピーに変換して依存を切る
- [ ] **Task S026-2-3**: POST/GET/DELETE /api/v1/volumes/{id}/snapshots, POST /api/v1/snapshots/{id}/clone
- [ ] **Task S026-2-4**: 結合テスト: ボリューム→スナップショット→クローン→削除拒否→フラット化→削除成功
- [ ] **Task S026-2-5**: `cirrusctl snapshot/clone`

---

## Sprint S027: ストレージドレイン + マイグレーション [ ]

ストレージバックエンドのライフサイクル管理とボリュームのライブマイグレーションが動作する。

### Story S027-1: バックエンドライフサイクル + ボリューム移行 [ ]

- [ ] **Task S027-1-1**: ステータス遷移: active → degraded → draining → readonly → retired
- [ ] **Task S027-1-2**: BackendDriver.MigrateVolume（同種バックエンド間）
- [ ] **Task S027-1-3**: 汎用ブロックコピー（異種バックエンド間、ホスト経由）
- [ ] **Task S027-1-4**: ドレインオーケストレーション: 依存関係考慮の移行順序算出、帯域制限、進捗追跡
- [ ] **Task S027-1-5**: 結合テスト: バックエンドドレイン→ボリューム順次移行→退役

---

## Sprint S028: テンプレートサービス [ ]

テンプレートの登録・公開・キャッシュコピーが動作し、VM 作成時のテンプレート選択が機能する。

### Story S028-1: テンプレートモデル + Service [ ]

- [ ] **Task S028-1-1**: マイグレーション: templates, template_caches テーブル
- [ ] **Task S028-1-2**: `internal/template/service.go` Create, EnsureCached
- [ ] **Task S028-1-3**: 公開範囲管理: public, organization, tenant

### Story S028-2: キャッシュ LRU 管理 + VM 統合 [ ]

- [ ] **Task S028-2-1**: last_used_at 更新 + LRU eviction（容量閾値ベース）
- [ ] **Task S028-2-2**: Compute.CreateVM: テンプレート指定時に EnsureCached → キャッシュ完了待ち → クローン
- [ ] **Task S028-2-3**: POST/GET/DELETE/PUT /api/v1/templates
- [ ] **Task S028-2-4**: 結合テスト: テンプレート作成→別バックエンドで VM 作成→キャッシュコピー→VM 起動
- [ ] **Task S028-2-5**: `cirrusctl template`

---

## Sprint S029: 監視・メトリクス + Phase 2 安定化 [ ]

Prometheus メトリクス、ヘルスチェック、宣言的トポロジの乖離検出が動作する。Phase 2 全機能が安定。

### Story S029-1: メトリクス + 乖離検出 [ ]

- [ ] **Task S029-1-1**: Prometheus exporter（/metrics エンドポイント）
- [ ] **Task S029-1-2**: ホスト/ストレージ/ネットワーク/スケジューラのメトリクス
- [ ] **Task S029-1-3**: 宣言トポロジと実態の乖離検出（ストレージ到達性、ネットワーク到達性、Capability 差異）

### Story S029-2: Phase 2 E2E テスト [ ]

- [ ] **Task S029-2-1**: ライブマイグレーション + DRS + ストレージドレインの複合シナリオ
- [ ] **Task S029-2-2**: ホストプロファイルロールアウト中の VM 可用性
- [ ] **Task S029-2-3**: cirrus-sim medium 環境での全機能テスト + 障害注入強化

---

## Sprint S030: Controller HA [ ]

Controller を複数インスタンス Active/Active 構成で運用でき、1台停止しても自動でリーダー切り替えが行われる。設計: docs/controller-ha.md 参照。

### Story S030-1: リーダー選出 [ ]

- [ ] **Task S030-1-1**: `internal/controller/leader.go` PostgreSQL アドバイザリーロックによるリーダー選出
- [ ] **Task S030-1-2**: /healthz にリーダーフラグ追加

### Story S030-2: シングルトンジョブのリーダー限定実行 [ ]

- [ ] **Task S030-2-1**: HeartbeatMonitor / ReconcileLoop / HostFaultyHandler / DRS: リーダーのみ起動/停止
- [ ] **Task S030-2-2**: `SELECT FOR UPDATE` による Scheduler 分散ロック

### Story S030-3: HA 対応 + テスト [ ]

- [ ] **Task S030-3-1**: Worker gRPC 接続のリトライロジック（agent 側）
- [ ] **Task S030-3-2**: PostgreSQL フェイルオーバー時の 503 レスポンス + pgxpool 自動リコネクト
- [ ] **Task S030-3-3**: 2台構成でリーダー選出 + 停止 → 切り替え + 同時スケジューリングで競合しないこと

---

## Sprint S031: Service Insertion（トラフィック経路挿入） [ ]

テナント Network のトラフィック経路にサービス VM（FW、IDS 等）を挿入できる。

### Story S031-1: Service Insertion 実装 [ ]

- [ ] **Task S031-1-1**: マイグレーション: service_insertions テーブル
- [ ] **Task S031-1-2**: テナント API: POST/GET/DELETE /tenants/{tid}/networks/{nid}/service-insertions
- [ ] **Task S031-1-3**: service_in / service_out ポートの自動作成
- [ ] **Task S031-1-4**: OpenFlow パイプラインに Service Insertion 分岐を追加（GW ノードへのステアリング）
- [ ] **Task S031-1-5**: ヘルスチェック: サービス VM 死活監視 + 障害時バイパス
- [ ] **Task S031-1-6**: テスト: 対象トラフィックがサービス VM 経由で転送されること

---

## Sprint S032: ゲートウェイ HA + スケールアウト [ ]

ゲートウェイノードの高可用性とスケールアウトが動作する。Active-Standby + BFD モデル。

### Story S032-1: GW HA + 無停止移動 + スケールアウト [ ]

- [ ] **Task S032-1-1**: BFD による死活監視 + Active → Standby 自動フェイルオーバー
- [ ] **Task S032-1-2**: GW ノードの無停止移動（Fallback パターン: drain フロー → 既存セッション自然タイムアウト待ち → 旧 GW から削除）
- [ ] **Task S032-1-3**: GW ペア追加による Network 割り当て再配分
- [ ] **Task S032-1-4**: テスト: 障害注入→BFD フェイルオーバー + 無停止移動 + スケールアウト

---

## Sprint S033: 複数ストレージドメイン + レプリケーション [ ]

複数ストレージドメインとリージョン間レプリケーションが動作する。

### Story S033-1: マルチストレージドメイン + レプリケーション [ ]

- [ ] **Task S033-1-1**: スケジューラ: ドメインを考慮したバックエンド選定
- [ ] **Task S033-1-2**: マイグレーション: replication_policies テーブル
- [ ] **Task S033-1-3**: レプリケーションポリシー定義（対象、宛先、頻度、保持世代数）
- [ ] **Task S033-1-4**: 定期レプリケーション実行 + 差分転送 capability 判定
- [ ] **Task S033-1-5**: テスト: cirrus-sim medium（2バックエンド: SSD/HDD）でレプリケーション確認

---

## Sprint S034: Service Endpoint（テナント間サービス公開） [ ]

テナントが自身のサービスを他テナントに公開し、消費側テナントが DNS 名で安全に接続できる。双方向 NAT によるIP アドレス空間の完全隔離。

### Story S034-1: Service Endpoint 実装 [ ]

- [ ] **Task S034-1-1**: マイグレーション: service_endpoints, endpoint_connections テーブル
- [ ] **Task S034-1-2**: テナント API: POST/GET/DELETE /tenants/{tid}/networks/{nid}/service-endpoints
- [ ] **Task S034-1-3**: CreateEndpointConnection: 消費側テナントが接続リクエスト + 承認フロー
- [ ] **Task S034-1-4**: 双方向 NAT（DNAT + SNAT）でテナント間 IP アドレス空間を完全隔離
- [ ] **Task S034-1-5**: DNS 統合: my-api.tenant-b.service.internal → VIP（100.127.0.0/16）
- [ ] **Task S034-1-6**: テスト: テナント A 公開 → テナント B 接続 → DNS 名で通信 + 双方向 NAT

---

## Sprint S035: Phase 3 安定化 [ ]

Phase 3 全機能の結合テストが通り、安定してデプロイできる状態。

### Story S035-1: 外部 IPAM 連携 + NetBox 同期 [ ]

- [ ] **Task S035-1-1**: IPAM インターフェースの外部実装（NetBox, Infoblox）
- [ ] **Task S035-1-2**: `internal/hook/netbox/` NetBox REST API 同期アダプタ（サイト/ラック/デバイス → Cirrus ロケーションツリー）

### Story S035-2: Phase 3 E2E テスト [ ]

- [ ] **Task S035-2-1**: Service Insertion + GW HA + レプリケーション + Service Endpoint の複合シナリオ
- [ ] **Task S035-2-2**: cirrus-sim medium/large 環境でのテスト

---

## Sprint S036: ファイルストレージ [ ]

NFS/CIFS 共有ボリュームが API で管理できる。

- [ ] **Task S036-1**: ファイルストレージバックエンドドライバ
- [ ] **Task S036-2**: 共有ボリュームの ACL 管理
- [ ] **Task S036-3**: VM からのマウント
- [ ] **Task S036-4**: テスト

---

## Sprint S037: オブジェクトストレージ連携 [ ]

S3 互換 API でオブジェクトストレージが使用でき、テンプレートのバックストアとして機能する。

- [ ] **Task S037-1**: MinIO 等の外部サービス連携
- [ ] **Task S037-2**: Cirrus 認証基盤との統合
- [ ] **Task S037-3**: テンプレートのオブジェクトストレージ保存
- [ ] **Task S037-4**: テスト

---

## Sprint S038: QoS + 帯域管理 [ ]

ボリューム QoS とネットワーク帯域管理が動作する。

- [ ] **Task S038-1**: ボリュームタイプ QoS ポリシーの実効化（バックエンドドライバ経由）
- [ ] **Task S038-2**: ネットワーク帯域制限（データプレーン QoS 機能）
- [ ] **Task S038-3**: noisy neighbor 制御のテスト

---

## Sprint S039: ポリシーエンジン連携 [ ]

OPA 等の外部ポリシーエンジンで認可判定が行える。

- [ ] **Task S039-1**: Authorizer インターフェースの OPA 実装
- [ ] **Task S039-2**: RBAC → OPA への段階的移行パス
- [ ] **Task S039-3**: リソース属性に基づく判定（ABAC サポート）
- [ ] **Task S039-4**: テスト

---

## Sprint S040: CMDB 同期 + Phase 4 安定化 [ ]

外部 CMDB との双方向同期が動作する。全 Phase 完了。

- [ ] **Task S040-1**: NetBox 双方向同期（Cirrus → NetBox のステータス反映）
- [ ] **Task S040-2**: 他 CMDB 対応（同期アダプタインターフェースの汎用化）
- [ ] **Task S040-3**: 全 Phase E2E テスト（cirrus-sim large 環境）
- [ ] **Task S040-4**: 負荷テスト: 2,500+ ホスト環境でのスケジューラ性能
- [ ] **Task S040-5**: ドキュメント最終更新
...
- S022 depends on S016（VM ライフサイクル完成）
- S023 depends on S022（マイグレーションインフラが必要）, S017（faulty 遷移トリガー）
- S024 depends on S022（MigrateVM が必要）
- S025 depends on S021（Phase 1 安定後に運用機能追加）
- S030 depends on S029（Phase 2 安定後に HA 追加）
- S031-S035 depends on S021（Phase 1 完了が前提）
