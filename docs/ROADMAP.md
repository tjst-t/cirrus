...
- Total: 40 Sprints | Done: 15 | In Progress: 0 | Remaining: 25
- [████████░░░░░░░░░░░░] 37%
...
### Story S016-4: make serve シード拡充 [ ]

`make serve` 後に追加手作業なしで VM 作成が通るよう、`_seed-topology` に以下を追加する。

- [ ] **Task S016-4-1**: `_seed-topology` に sim StorageBackend 作成を追加（driver=sim、storage-domain=default-sd）
- [ ] **Task S016-4-2**: `_seed-topology` に default VolumeType 作成を追加
- [ ] **Task S016-4-3**: `_activate-hosts` または `_seed-topology` に ホスト→default-sd 関連付けを追加（登録済み全ホスト対象）

### Story S016-5: テスト + CLI [ ]

- [ ] **Task S016-5-1**: 結合テスト: 作成→停止→起動→再起動→削除の全ライフサイクル
- [ ] **Task S016-5-2**: 異常系: 作成途中の worker 障害で error ステータス遷移
- [ ] **Task S016-5-3**: cirrus-sim 障害注入: libvirt-sim の DomainCreate 失敗
- [ ] **Task S016-5-4**: `cirrusctl vm start/stop/reboot/delete`
