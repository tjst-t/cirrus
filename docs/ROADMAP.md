...
- Total: 41 Sprints | Done: 19 | In Progress: 0 | Remaining: 22
- [█████████░░░░░░░░░░░] 46%

## Execution Order

S001 → S002 → S003 → S004 → S005 → S006 → S007 → S008 → S009 → S010 → S011 → S012 → S013 → S014 → S015 → S016 → S017 → S018 → S019 → S020 → S021 → S022 → S023 → S024 → S025 → S026 → S027 → S028 → S029 → S030 → S031 → S032 → S033 → S034 → S035 → S036 → S037 → S038 → S039 → S040 → S041
                                                                                                                                                          ↑ next
...
- [ ] **Task S020-4-6**: Egress/Ingress 数クォータチェックを Quota.Check/Reserve/Commit/Release に組み込み（S019 で対象外とした分）
...
### Story S025-1: DRS エンジン + 実行 [ ]

- [ ] **Task S025-1-1**: DRS ポリシー定義（CPU/メモリ使用率の標準偏差閾値、実行間隔、最大同時マイグレーション数）
- [ ] **Task S025-1-2**: `internal/scheduler/drs.go` リソース偏りの検出 + マイグレーション計画生成
- [ ] **Task S025-1-3**: 定期実行（configurable interval）、同時マイグレーション数制限
- [ ] **Task S025-1-4**: テスト: ホスト間のリソース偏り→DRS 実行→分布改善確認
...
- S022 depends on S021（Phase 1 全機能完了が前提）
