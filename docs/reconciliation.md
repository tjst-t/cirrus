...
| Network | OVN LSP/ACL/Router/NAT Ruleが消えている | OVNに未知のリソースがある |
...
| Network | ポートバインディング不一致 | OVNのchassisが期待するhost_idと異なる |
| Network | ACL不一致 | DB上のSGルールとOVN ACLが不一致 |
...
reconcile loop:
  Network (デフォルト5分間隔):
    1. OVN NB をスキャン（Logical Switch, LSP, ACL, Router, NAT Rule）
    2. DB上のnetworks/ports/security_groups/routers/floating_ipsと照合
    3. 差分をDriftEventとして発火
    ※ 遷移中ステータスのリソースに紐づくOVNオブジェクトは除外
...
- **ポート再バインド**: OVNのchassisが間違っている → 正しいhost_idに更新
...
Controller
  ...
  │     ├── OVNReconciler        [Sprint 5 → 9, 11で拡張]
  ...
  │     ├── AutoHealer           VMステータス同期、ポート再バインド等
...
type DriftEvent struct {
    ...
    Resource    string    // リソース種別 ("vm", "port", "volume", "lsp", "acl", "router", "nat_rule")
    ResourceID  string    // リソースID
    TenantID    string    // テナントID（影響範囲の特定用、ホスト系はempty）
    HostID      string    // 関連ホスト（あれば）
    Expected    string    // DBの状態
    Actual      string    // 実際の状態
    DetectedBy  string    // "heartbeat_reconciler", "ovn_reconciler", "storage_reconciler",
...
| network | expected_missing | lsp | critical | Alert | |
| network | unexpected_present | lsp | high | Alert → 段階的対応 | |
| network | state_mismatch | binding | high | Auto-heal: OVN chassis再バインド | |
| network | expected_missing | acl | critical | Alert | |
| network | expected_missing | router | critical | Alert | |
| network | expected_missing | nat_rule | critical | Alert（FIP断） | |
...
| `reconcile-network-interval` | 5m | OVN reconcileの間隔（未指定時はreconcile-interval） |
