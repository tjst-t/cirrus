...
#### S1-1: プロジェクト初期化
- [ ] Go module初期化、ディレクトリ構成作成（architecture.mdのディレクトリ構成に準拠）
- [ ] cmd/cirrus/main.go: controller/workerサブコマンド、設定ファイル読み込み
- [ ] internal/config: 設定ファイルパース（controller.yaml / worker.yaml）
- [ ] Makefile: build, test, lint, serve, logs ターゲット
- [ ] .gitignore, cirrus.yaml.example

#### S1-2: State モジュール基盤
- [ ] internal/state/db.go: PostgreSQL接続、コネクションプール
- [ ] internal/state/migrations/: golang-migrate導入、初回マイグレーション（空のスキーマバージョンテーブル）
- [ ] docker-compose.yaml: PostgreSQL

#### S1-3: API骨格
- [ ] internal/api/router.go: HTTPルーター初期化（chi or 標準ライブラリ）
- [ ] internal/api/middleware.go: リクエストID、ログ、リカバリ
- [ ] GET /healthz エンドポイント（DB接続チェック）

#### S1-4: gRPC骨格
- [ ] proto/agent.proto: Heartbeat RPCのみ定義
- [ ] proto生成（buf or protoc）
- [ ] internal/agent/agent.go: gRPCサーバ起動、Heartbeat実装（ログ出力のみ）
- [ ] controller側: workerへのgRPC接続管理

#### S1-5: cirrus-sim接続確認
- [ ] worker起動時にcirrus-sim（libvirt-sim）へTCP接続確認
- [ ] controller起動時にcirrus-sim（ovn-sim）へOVSDB接続確認
- [ ] make serve でcontroller + worker + cirrus-simが連携起動するスクリプト
- [ ] 結合テスト: healthzが200を返す

**デプロイ確認**: `make serve` → controller + worker起動 → healthz OK → cirrus-sim接続OK
...
#### S2-1: テナントモデルのDB
- [ ] マイグレーション: organizations, tenants, users, role_assignments テーブル
- [ ] internal/state/models.go: Organization, Tenant, User, RoleAssignment 構造体

#### S2-2: Identity Service
- [ ] internal/identity/service.go: TenantService インターフェース定義
- [ ] internal/identity/tenant.go: CreateOrganization, CreateTenant, AssignRole 実装
- [ ] テスト: 組織作成→テナント作成→ロール付与

#### S2-3: 認証
- [ ] internal/identity/authenticator.go: Authenticator インターフェース定義
- [ ] 静的トークン認証実装（開発用）: 設定ファイルでtoken→user_idマッピング
- [ ] internal/api/middleware.go: 認証ミドルウェア（Authorizationヘッダ解析）

#### S2-4: 認可
- [ ] internal/identity/authorizer.go: Authorizer インターフェース定義
- [ ] RBAC実装: infra_admin, org_admin, tenant_admin, tenant_member
- [ ] internal/api/middleware.go: X-Tenant-IDヘッダからテナントスコープ解決

#### S2-5: APIエンドポイント
- [ ] POST/GET /api/v1/organizations
- [ ] POST/GET /api/v1/organizations/{org_id}/tenants
- [ ] POST/GET/DELETE /api/v1/tenants/{id}/role-assignments
- [ ] 結合テスト: 認証→認可→テナント操作の一連フロー
...
#### S3-1: HostモデルのDB
- [ ] マイグレーション: hosts, host_storage_domains テーブル
- [ ] capability JSONB、resource_physical JSONB、overcommit_ratios JSONB

#### S3-2: Host Service
- [ ] internal/host/service.go: Service インターフェース定義
- [ ] internal/host/manager.go: Register, UpdateCapability, SetOperationalState, Heartbeat
- [ ] internal/host/manager.go: GetAllocatable, GetAllocated（スケジューラ向け）

#### S3-3: Worker Agent
- [ ] proto/agent.proto: Heartbeat RPC にResourceReport追加
- [ ] internal/agent/agent.go: 定期heartbeat送信（リソース情報含む）
- [ ] controller側: heartbeat受信→hosts テーブル更新

#### S3-4: Hypervisor接続
- [ ] internal/hypervisor/driver.go: Driver インターフェース定義
- [ ] internal/hypervisor/libvirt/libvirt.go: go-libvirt接続、ListVMs
- [ ] worker起動時にcirrus-sim（libvirt-sim）に接続し、ホスト情報取得

#### S3-5: APIエンドポイント + テスト
- [ ] POST/GET /api/v1/hosts（インフラ管理者）
- [ ] POST /api/v1/hosts/{id}/actions（maintenance, activate）
- [ ] 結合テスト: worker起動→heartbeat→ホスト一覧にリソース情報表示
