...
#### S3.5-1: 登録トークン
- [ ] internal/config: controller に `--registration-token` フラグ追加
- [ ] 登録トークンは共有シークレット（worker設定ファイルに記載）
- [ ] Makefile: AUTH_TOKENS と同様に `REGISTRATION_TOKEN` 変数で注入

#### S3.5-2: Worker自己登録 gRPC
- [ ] proto/agent.proto: RegisterHost RPC 追加（registration_token, hostname, address, capability）
- [ ] worker起動時: ホスト情報（hostname, address, libvirt経由のcapability）を収集
- [ ] worker起動時: RegisterHost を呼び出し、自ホストをcontrollerに登録
- [ ] controller側: トークン検証 → hosts テーブルに registering 状態で INSERT（既存なら無視）
- [ ] RegisterHost レスポンスで割り当てられた host UUID を返却
- [ ] worker は以降の heartbeat で UUID を使用（名前マッチ廃止）

#### S3.5-3: Heartbeat を UUID ベースに移行
- [ ] worker: RegisterHost で取得した UUID を heartbeat の host_id に使用
- [ ] controller: heartbeat は UUID のみでマッチ（名前マッチを廃止）
- [ ] 未登録ホストからの heartbeat は拒否（accepted=false）

#### S3.5-4: 管理者承認フロー
- [ ] registering 状態のホスト一覧表示（`cirrusctl admin host list --pending`）
- [ ] `cirrusctl admin host activate` で registering → active に承認
- [ ] 承認前のホストはスケジューラの配置対象外

#### S3.5-5: Makefile 対応
- [ ] `_register-hosts` ターゲットを削除（worker自己登録に移行）
- [ ] worker 起動時に `--registration-token` を渡す
- [ ] worker 起動後に自動で全ホストを activate する開発用ステップ追加

#### S3.5-6: テスト
- [ ] 無効なトークンでの登録拒否テスト
- [ ] worker起動→自動登録→管理者activate→heartbeat正常の一連フロー
- [ ] 同一ホスト名での重複登録が冪等であることの確認
...
### Sprint 8.5: ホスト状態遷移制約 + Heartbeat監視

**ゴール**: ホストの operational_state 遷移に制約を適用し、heartbeat途絶でfaulty自動遷移、draining完了で自動maintenance遷移が動作する。
...
**デプロイ確認**: 不正遷移が拒否される + heartbeat停止→faulty自動遷移 + drain完了→maintenance自動遷移
