...
- [ ] internal/controller/reconcile/network.go: OVNReconciler実装
- [ ] reconcile loop（デフォルト5分間隔）でOVN NBをスキャン（Logical Switch, LSP）
- [ ] DB上のnetworks/portsと照合
- [ ] 初期実装はログ出力のみ（DriftEvent基盤はSprint 8.5で完成後に移行）
- [ ] 遷移中ステータスのリソースはreconcile対象外とする
