...
### Sprint 5.5: テナント向けリソース抽象化（AZ + Network API改善）

**ゴール**: Availability Zone（AZ）を導入し、テナント API からインフラ詳細（network_domain_id 等）を隠蔽する。テナントは AZ 名とリソース名だけで操作できる。設計詳細は [docs/tenant-model.md](tenant-model.md) を参照。

#### S5.5-1: Availability Zone モデル
- [ ] マイグレーション: availability_zones, az_storage_domains テーブル
- [ ] internal/az/models.go: AvailabilityZone 構造体
- [ ] internal/az/service.go: Service インターフェース定義
- [ ] internal/az/store.go: CRUD実装

#### S5.5-2: AZ 管理者 API + CLI
- [ ] POST/GET/PUT/DELETE /api/v1/availability-zones（管理者）
- [ ] POST/DELETE /api/v1/availability-zones/{id}/storage-domains（SD紐付け）
- [ ] cirrusctl admin az create/list/show/delete コマンド

#### S5.5-3: AZ テナント API
- [ ] GET /api/v1/availability-zones（テナント向け: 利用可能AZ一覧）
- [ ] GET /api/v1/availability-zones/{id}（AZ詳細: 名前、説明）
- [ ] RBAC: 全テナントロールで AZ 一覧参照可能

#### S5.5-4: Network API から network_domain_id を除去
- [ ] POST /api/v1/networks: network_domain_id を az パラメータに変更（optional）
- [ ] az 指定時 → AZ の network_domain_id を自動使用
- [ ] az 未指定時 → デフォルト AZ（Phase 1 では唯一の AZ）を使用
- [ ] CLI: cirrusctl network create から --network-domain を除去、--az を追加（optional）

#### S5.5-5: make serve での AZ 自動シード
- [ ] Makefile: トポロジシード時にデフォルト AZ を自動作成（Location + ND + SD 紐付け）
- [ ] 既存の default-nd, default-sd, default-site を AZ に集約

#### S5.5-6: テスト
- [ ] AZ CRUD テスト
- [ ] ネットワーク作成が AZ 経由で network_domain を自動解決すること
- [ ] AZ 未指定でデフォルト AZ が使われること
- [ ] cirrus-sim 結合テスト: AZ 名指定でネットワーク作成→OVN反映
...
**ゴール**: 統一的なDriftEvent基盤を構築し、heartbeat内のVM情報でCompute層のドリフトをパッシブ検出する。Sprint 5/6で実装済みのOVN/Storage ReconcilerをDriftEvent発火に移行する。
...
- [ ] Sprint 5のOVN ReconcilerをDriftEvent発火に移行
...
#### S9-5: OVN Reconciler ACL拡張（docs/reconciliation.md 参照）
- [ ] OVNReconcilerにACLスキャン追加
- [ ] DB上のsecurity_group_rulesとOVN ACLを照合 → DriftEvent発火
- [ ] ACL消失（DB有・OVN無）→ Alert（critical）
- [ ] 未知ACL（DB無・OVN有）→ Alert（high）
...
**デプロイ確認**: SG作成→ルール追加→VM作成時にSG指定→ovn-simでACL確認 + ACL削除→Reconciler検出
...
#### S11-5: OVN Reconciler Router/NAT/FIP拡張（docs/reconciliation.md 参照）
- [ ] OVNReconcilerにLogical Router, Router Port, NAT Ruleスキャン追加
- [ ] DB上のrouters/router_interfaces/floating_ipsとOVNを照合 → DriftEvent発火
- [ ] Router消失 → Alert（critical）
- [ ] NAT Rule消失（FIP断）→ Alert（critical）
- [ ] 未知Router/NAT Rule → Alert（high）

#### S11-6: APIエンドポイント + テスト
...
**デプロイ確認**: ルータ+FIP構成→ovn-simでLogical Router + NAT Rule確認 + NAT Rule削除→Reconciler検出
...
- [ ] Reconcile E2Eテスト: OVN上のLSP/ACLを手動削除 → OVNReconcilerがDriftEvent発火 → Alert確認
...
- [ ] 各Reconciler（S5-5, S6-5, S8.5b, S9-5, S11-5）の統合動作確認
...
- [ ] ReconcileLoop（OVN/Storage）: リーダーのみ起動/停止
...
### Sprint 20: 複数ネットワークドメイン（マルチAZ）

**ゴール**: 複数のOVNクラスタを複数AZとして管理し、テナントがAZ指定でリソースを配置できる。

#### S20-1: マルチOVNクライアント
- [ ] network_domainsテーブルの各レコードからOVN NB接続を管理
- [ ] OVNClientをドメインごとにインスタンス化（接続プール）
- [ ] ネットワーク作成時にAZ指定 → AZのnetwork_domain_idで自動解決

#### S20-2: マルチAZ運用
- [ ] 複数AZ作成（tokyo-1, osaka-1）、各AZに独立したND/SD紐付け
- [ ] テナントネットワークはAZ内の1つのnetwork_domainに所属
- [ ] 同一AZ内のポート作成のみ許可（AZ跨ぎはRouter経由、Sprint 21）
- [ ] テナントAPI: AZ一覧でリソース概要（ホスト数等）表示

#### S20-3: テスト
- [ ] cirrus-sim medium環境（2 OVNクラスタ: 東京/大阪 = 2 AZ）
- [ ] 各AZで独立にネットワーク/VM作成
- [ ] AZ間の隔離確認（L2不可、L3はSprint 21で対応）

**デプロイ確認**: 2つのAZで独立にVM運用可能

---

### Sprint 21: OVN Interconnect（AZ間L3接続）

**ゴール**: OVN-ICによりAZ間のL3ルーティングが動作する。テナントはRouter作成でAZ間を接続する。
...
#### S21-2: AZ間ネットワーク接続
- [ ] テナントRouterによるAZ間ルーティング（OVN-IC経由L3）
- [ ] AZ-AのサブネットとAZ-BのサブネットをRouterで接続
- [ ] AZ間のVM通信（L3ルーティング）

#### S21-3: テスト
- [ ] cirrus-sim medium環境でAZ間L3ルーティング
- [ ] AZ-A/AZ-BのVM間通信確認（Router経由）

**デプロイ確認**: 東京/大阪のAZ間でRouter経由のVM間L3通信
...
│   │   ├── Sprint 4 (Topology)
│   │   │   ├── Sprint 5 (Network)
│   │   │   │   ├── Sprint 5.5 (AZ + テナント抽象化)
│   │   │   │   ├── Sprint 9 (SG)
│   │   │   │   └── Sprint 11 (Router + FIP)
│   │   │   └── Sprint 6 (Storage) ← Sprint 5.5にも依存（AZ経由のバックエンド解決）
│   │   │       └── Sprint 7 (Scheduler + VM作成 + Flavor) ← Sprint 5.5にも依存
│   │   │           ├── Sprint 8 (VMライフサイクル)
│   │   │           └── Sprint 10 (Quota)
...
│
├── Sprint 20 (マルチAZ) ← Sprint 12, Sprint 5.5
│   └── Sprint 21 (OVN-IC / AZ間L3)
├── Sprint 22 (マルチストレージ + レプリケーション) ← Sprint 12, 17, Sprint 5.5
